...
void Foam::IH_Waves_InletAlphaFvPatchScalarField::updateCoeffs()
{
    ...
    // PHC //
    Info << alphaName() << " BC on patch " << this->patch().name() << endl;
    ...
    scalarField patchVOF = alphaCell; // Output variable
    ...
    if ( noEta && genAbs_ == false ) // zeroGradient (no eta specified)
    {
        patchVOF = alphaCell;
    }
    else
    {
        forAll(patchHeight, cellIndex)    
        {
            ...
            ) // T > M - Inflow correction
            {
                if (zSup[cellIndex] <= measuredLevels[cellGroup[cellIndex]-1])
                {// Full cell
                    if ( neg(UCell[cellIndex] & nVecCell[cellIndex]) )
                    {// In flux
                        patchVOF[cellIndex] = 1.0;
                    }
                    else
                    {
                        patchVOF[cellIndex] = alphaCell[cellIndex];
                    }
                }
                else if 
                ( zInf[cellIndex] < measuredLevels[cellGroup[cellIndex]-1])
                {// Partial cell
                    auxiliar = 
                        calculatedLevel[cellGroup[cellIndex]-1] - 
                        zInf[cellIndex];
                    ...
                    if ( neg(UCell[cellIndex] & nVecCell[cellIndex]) )
                    {// In flux
                        //patchVOF[cellIndex] = max(auxiliar/auxiliarTotal,alphaCell[cellIndex]);
                        patchVOF[cellIndex] = 1.0;
                    }
                    else
                    {
            ...
            else // M > T - Outflow correction
            {
                ...
                (zSup[cellIndex] <= measuredLevels[cellGroup[cellIndex]-1]) 
                {// Water over theoretical (reflection)
                    patchVOF[cellIndex] = alphaCell[cellIndex];
                }
                else if 
                (zInf[cellIndex] < measuredLevels[cellGroup[cellIndex]-1]) 
